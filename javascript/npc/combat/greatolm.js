...
cs = new NCombatScript() {
    ...
    /* @Override */
    setVariable: function(name, object) {
        if (name.equals("olm")) {
            olm = object;
    ...
    restore: function() {
        ...
        this.despawn(LEFT_HAND, 0, true);
        this.despawn(RIGHT_HAND, 0, true);
        if (olm != null) {
    ...
    applyDeadStartHook: function(deathDelay) {
        ...
        this.despawn(index, DESPAWN_TIMES[index], false);
        if (index == HEAD) {
            for each (var player in npc.getController().getPlayers()) {
    ...
    despawn: function(index, time, force) {
        if (time != 0) {
    ...
    rotateStart: function() {
        ...
        olm[LEFT_HAND].getCombat().getCombatScript().setVariable("phase", phase);
        olm[RIGHT_HAND].getCombat().getCombatScript().setVariable("phase", phase);
        this.despawn(HEAD, DESPAWN_TIMES[HEAD], false);
        ...
        var event = new Event(Event.MILLIS_1200) {
            execute: function() {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    var the = new TileHitEvent(Event.MILLIS_3600, npc.getController(), tile, 20, Hit.Type.TYPELESS);
    ...
    rotate: function(west) {
        ...
        for each (var hand in hands) {
            ...
            hand.getCombat().getCombatScript().setVariable("olm", olm);
        }
        ...
        olm[LEFT_HAND].getCombat().getCombatScript().setVariable("objects", objects);
