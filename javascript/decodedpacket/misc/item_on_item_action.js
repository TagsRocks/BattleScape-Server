DecodedPacketItemOnItem.prototype.doAction = function(player, useInterfaceID, useChildID, onInterfaceID,
        onChildID, useSlot, onSlot, useItemID, onItemID) {
    if (useInterfaceID == WidgetID.INVENTORY && onInterfaceID == WidgetID.INVENTORY) {
        ...
        var useItem = player.getInventory().getItem(useSlot);
        var onItem =  player.getInventory().getItem(onSlot);
        if (this.used(useItemID, onItemID, ItemID.DRAKES_TOOTH, ItemID.HOLY_SANDALS)) {
        ...
        } else if (this.used(useItemID, onItemID, ItemID.DRAKES_CLAW, ItemID.BOOTS_OF_STONE)) {
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(ItemID.BOOTS_OF_BRIMSTONE, 1, onSlot);
        } else if (this.used(useItemID, onItemID, ItemID.ZAMORAKIAN_HASTA, ItemID.HYDRAS_CLAW)) {
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(ItemID.DRAGON_HUNTER_LANCE, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 4151, 12004)) {
            ...
            onItem.setAttachment(Math.min(onItem.getDef().getDegradeTime(), onItem.getCharges()
                    + useItem.getCharges()));
            player.getInventory().deleteItem(useItemID, 1, useSlot);
        } else if (this.used(useItemID, onItemID, 22486, 22446) || this.used(useItemID, onItemID, 22325, 22446)) {
            // Scythe of vitur/(uncharged) and Vial of blood
            var vialOfBloodCount = player.getInventory().getCount(22446);
            if (player.getInventory().getCount(565) < 300 * vialOfBloodCount) {
                player.getGameEncoder().sendMessage("You need " + (300 * vialOfBloodCount)
                        + " blood runes to do this.");
                return;
            }
            var chargingSlot = (useItemID == 22325 ||  useItemID == 22486) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(22325, chargingSlot, vialOfBloodCount,
                    new Item(22446, 1), 100);
            if (charges > 0) {
                player.getInventory().deleteItem(565, 300 * charges);
            }
        } else if (this.used(useItemID, onItemID, 11908, 12004)) {
            if (player.getInventory().getCount(12004) < 10) {
                player.getGameEncoder().sendMessage("You need 10 kraken tentacles to do this.");
                return;
            }
            var chargingSlot = useItemID == 11908 ? useSlot : onSlot;
        ...
        } else if (this.used(useItemID, onItemID, 11908, 560) || this.used(useItemID, onItemID, 11907, 560)
                || this.used(useItemID, onItemID, 11908, 562) || this.used(useItemID, onItemID, 11907, 562)
                || this.used(useItemID, onItemID, 11908, 554) || this.used(useItemID, onItemID, 11907, 554)) {
            // Trident of the seas
            var deathRuneCount = player.getInventory().getCount(560);
            var chaosRuneCount = player.getInventory().getCount(562);
            var fireRuneCount = player.getInventory().getCount(554);
            var coinsCount = player.getInventory().getCount(ItemID.COINS);
            var maxCharges = Math.min(deathRuneCount, chaosRuneCount);
            maxCharges = Math.min(maxCharges, fireRuneCount / 5);
            maxCharges = Math.min(maxCharges, coinsCount / 10);
            if (maxCharges == 0) {
                player.getGameEncoder().sendMessage("Each charge requires 1 death rune, 1 chaos rune, 5 fire runes and 10 coins.");
                return;
            }
            var chargingSlot = (useItemID == 11907 ||  useItemID == 11908) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(11907, chargingSlot, maxCharges,
                    new Item(560, 1), 1);
            if (charges > 0) {
            ...
            var chargedItem = player.getInventory().getItem(chargingSlot);
        ...
        } else if (this.used(useItemID, onItemID, 22290, 560) || this.used(useItemID, onItemID, 22288, 560)
                || this.used(useItemID, onItemID, 22290, 562) || this.used(useItemID, onItemID, 22288, 562)
                || this.used(useItemID, onItemID, 22290, 554) || this.used(useItemID, onItemID, 22288, 554)) {
            ...
            var chargingSlot = (useItemID == 22288 ||  useItemID == 22290) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(22288, chargingSlot, maxCharges,
                    new Item(560, 1), 1);
            if (charges > 0) {
                player.getInventory().deleteItem(562, charges);
                player.getInventory().deleteItem(554, charges * 5);
            ...
            var chargedItem = player.getInventory().getItem(chargingSlot);
        } else if (this.used(useItemID, onItemID, 12900, 560) || this.used(useItemID, onItemID, 12899, 560)
                || this.used(useItemID, onItemID, 12900, 562) || this.used(useItemID, onItemID, 12899, 562)
                || this.used(useItemID, onItemID, 12900, 554) || this.used(useItemID, onItemID, 12899, 554)
                || this.used(useItemID, onItemID, 12900, 12934) || this.used(useItemID, onItemID, 12899, 12934)) {
            // Trident of the swamp
            var deathRuneCount = player.getInventory().getCount(560);
            var chaosRuneCount = player.getInventory().getCount(562);
            var fireRuneCount = player.getInventory().getCount(554);
            var ZulrahScaleCount = player.getInventory().getCount(12934);
            var coinsCount = player.getInventory().getCount(ItemID.COINS);
            var maxCharges = Math.min(deathRuneCount, chaosRuneCount);
            maxCharges = Math.min(maxCharges, fireRuneCount / 5);
            maxCharges = Math.min(maxCharges, coinsCount / 10);
            ...
            if (maxCharges == 0) {
                player.getGameEncoder().sendMessage("Each charge requires 1 death rune, 1 chaos rune, 5 fire runes, 10 coins and 1 Zulrah's scale.");
                return;
            }
            var chargingSlot = (useItemID == 12899 ||  useItemID == 12900) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(12899, chargingSlot, maxCharges,
                    new Item(560, 1), 1);
            if (charges > 0) {
                player.getInventory().deleteItem(562, charges);
                player.getInventory().deleteItem(554, charges * 5);
        ...
        } else if (this.used(useItemID, onItemID, 22294, 560) || this.used(useItemID, onItemID, 22292, 560)
                || this.used(useItemID, onItemID, 22294, 562) || this.used(useItemID, onItemID, 22292, 562)
                || this.used(useItemID, onItemID, 22294, 554) || this.used(useItemID, onItemID, 22292, 554)
                || this.used(useItemID, onItemID, 22294, 12934) || this.used(useItemID, onItemID, 22292, 12934)) {
            ...
            var chargingSlot = (useItemID == 22292 ||  useItemID == 22294) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(22292, chargingSlot, maxCharges,
                    new Item(560, 1), 1);
            if (charges > 0) {
                player.getInventory().deleteItem(562, charges);
                player.getInventory().deleteItem(554, charges * 5);
        ...
        } else if (this.used(useItemID, onItemID, 22481, 565) || this.used(useItemID, onItemID, 22323, 565)) {
            // Sanguinesti staff/(uncharged) and Vial of blood
            var chargingSlot = (useItemID == 22323 ||  useItemID == 22481) ? useSlot : onSlot;
            var charges = player.getCharges().chargeFromInventory(22323, chargingSlot,
                    player.getInventory().getCount(565), new Item(565, 3), 1);
        } else if (this.used(useItemID, onItemID, 12954, 22477)) {
            // Dragon defender and Avernic defender hilt
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(22322, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 21907, 10499)) {
            // Vorkath's head and Ava's accumulator
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(22109, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 2425, 10499)) {
            // Vorkath's head and Ava's accumulator
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(22109, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 11791, 13256)) {
            // Staff of the dead and Saradomin's light
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(22296, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 11838, 12804)) {
        ...
        } else if (this.used(useItemID, onItemID, 1540, 11286)) {
        ...
        } else if (this.used(useItemID, onItemID, 2890, 21637)) {
            // Elemental shield and Wyvern visage
            if (player.getSkills().getLevel(Skills.SMITHING) < 66) {
                player.getGameEncoder().sendMessage("You need a Smithing level of 66 to do this.");
                return;
            } else if (player.getSkills().getLevel(Skills.MAGIC) < 66) {
                player.getGameEncoder().sendMessage("You need a Magic level of 66 to do this.");
                return;
            }
            player.getSkills().addXP(Skills.SMITHING, 2000);
            ...
            player.getGameEncoder().sendMessage("At a great cost to your personal magical energies, you have crafted the wyvern visage and elemental shield into an ancient wyvern shield.");
        } else if (this.used(useItemID, onItemID, 1540, 22006)) {
        ...
        } else if (this.used(useItemID, onItemID, 1187, 22100)) {
            // Dragon sq shield and Dragon metal slice
            if (player.getSkills().getLevel(Skills.SMITHING) < 75) {
                player.getGameEncoder().sendMessage("You need a Smithing level of 75 to do this.");
                return;
            }
        ...
        } else if (this.used(useItemID, onItemID, 3140, 22103)) {
        ...
        } else if (this.used(useItemID, onItemID, 12829, 12833)) {
            // Spirit shield and Holy elixer
            if (player.getSkills().getLevel(Skills.PRAYER) < 85) {
                player.getGameEncoder().sendMessage("You need a Prayer level of 85 to do this.");
                return;
            }
        ...
        } else if (this.used(useItemID, onItemID, 12831, 12819)) {
        ...
        } else if (this.used(useItemID, onItemID, 12831, 12823)) {
        ...
        } else if (this.used(useItemID, onItemID, 12831, 12827)) {
        ...
        } else if (this.used(useItemID, onItemID, 11864, 7979)) {
        ...
        } else if (this.used(useItemID, onItemID, 11865, 7979)) {
        ...
        } else if (this.used(useItemID, onItemID, 11864, 7981)) {
        ...
        } else if (this.used(useItemID, onItemID, 11865, 7981)) {
        ...
        } else if (this.used(useItemID, onItemID, 11864, 7980)) {
         ...
         } else if (this.used(useItemID, onItemID, 11865, 7980)) {
        ...
        } else if (this.used(useItemID, onItemID, 11864, 21275)) {
         ...
         } else if (this.used(useItemID, onItemID, 11865, 21275)) {
        ...
        } else if (this.used(useItemID, onItemID, 11864, 21907)) {
         ...
         } else if (this.used(useItemID, onItemID, 11865, 21907)) {
        ...
        } else if (this.used(useItemID, onItemID, ItemID.SLAYER_HELMET, ItemID.ALCHEMICAL_HYDRA_HEADS)) {
         ...
         } else if (this.used(useItemID, onItemID, ItemID.SLAYER_HELMET_I, ItemID.ALCHEMICAL_HYDRA_HEADS)) {
            if (!player.getSlayer().isUnlocked(Slayer.USE_MORE_HEAD)) {
                player.getGameEncoder().sendMessage("You need to unlock this feature through Slayer.");
                return;
            }
        ...
        } else if (this.used(useItemID, onItemID, 1755, 12922)) {
            // Chisel and Tanzanite fang
            if (player.getSkills().getLevel(Skills.FLETCHING) < 53) {
                player.getGameEncoder().sendMessage("You need a Fletching level of 53 to do this.");
                return;
            }
        ...
        } else if (this.used(useItemID, onItemID, 13200, 12929)) {
        ...
        } else if (this.used(useItemID, onItemID, 13200, 12931)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 13201, 12929)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 13201, 12931)) {
        ...
        } else if (this.used(useItemID, onItemID, 19589, 19592) || this.used(useItemID, onItemID, 19589, 19601)) {
            ...
            if (!player.getInventory().hasItem(19592)) {
                ...
                player.getGameEncoder().sendMessage("You need a Fletching level of 72 to do this.");
                return;
            }
        ...
        } else if (this.used(useItemID, onItemID, 19604, 19610)) {
        ...
        } else if (this.used(useItemID, onItemID, 19607, 19610)) {
        ...
        } else if (this.used(useItemID, onItemID, 19595, 19610)) {
            ...
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(19478, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 19598, 19610)) {
            ...
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(19481, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 13227, 6920)) {
            // Eternal crystal and Infinity boots
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(13235, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 13229, 2577)) {
            // Pegasian crystal and Ranger boots
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(13237, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 13231, 11840)) {
            // Primordial crystal and Dragon boots
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(13239, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 21043, 6914)) {
            // Kodai insignia and Master wand
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(21006, 1, onSlot);
            player.getGameEncoder().sendMessage("Power shoots down the length of the wand, empowering it with the darkness of Xeric's Kodai wizards.");
        } else if (this.used(useItemID, onItemID, 985, 987)) {
            // Tooth half of key and Loop half of key
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(989, 1, onSlot);
        } else if (this.used(useItemID, onItemID, 269, 2436) || this.used(useItemID, onItemID, 269, 2440)
                || this.used(useItemID, onItemID, 269, 2442)) {
            Herblore.openMix(player, 12695);
        } else if (this.used(useItemID, onItemID, 3024, 235) || this.used(useItemID, onItemID, 3024, 1526)
                || this.used(useItemID, onItemID, 3024, 10937)) {
            Herblore.openMix(player, 10925);
        } else if (this.used(useItemID, onItemID, 13233, 6739) || this.used(useItemID, onItemID, 13233, 13242)
                || this.used(useItemID, onItemID, 13233, 13241)) {
            // Smouldering stone and Dragon axe/Infernal axe
            if (player.getController().getLevelForXP(Skills.FIREMAKING) < 85) {
                player.getGameEncoder().sendMessage("You need a Firemaking level of 85 to do this.");
                return;
            }
            var chargedItemID = 13241;
            var chargingSlot = (useItemID == 6739 || useItemID == 13242 || useItemID == chargedItemID)
                    ? useSlot : onSlot;
            var chargeWithItemID = 13233;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
                player.setAnimation(4512);
            }
        } else if (this.used(useItemID, onItemID, 6739, 13241) || this.used(useItemID, onItemID, 6739, 13242)) {
            ...
            var chargedItemID = 13241;
            var chargingSlot = (useItemID == 13242 || useItemID == chargedItemID) ? useSlot : onSlot;
            var chargeWithItemID = 6739;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
                player.setAnimation(4512);
            }
        } else if (this.used(useItemID, onItemID, 13233, 21028) || this.used(useItemID, onItemID, 13233, 21033)
                || this.used(useItemID, onItemID, 13233, 21031)) {
            // Smouldering stone and Dragon harpoon/Infernal harpoon
            if (player.getController().getLevelForXP(Skills.COOKING) < 85) {
                player.getGameEncoder().sendMessage("You need a Cooking level of 85 to do this.");
                return;
            }
            var chargedItemID = 21031;
            var chargingSlot = (useItemID == 21028 || useItemID == 21033 || useItemID == chargedItemID)
                    ? useSlot : onSlot;
            var chargeWithItemID = 13233;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
        ...
        } else if (this.used(useItemID, onItemID, 21028, 21031) || this.used(useItemID, onItemID, 21028, 21033)) {
            ...
            var chargedItemID = 21031;
            var chargingSlot = (useItemID == 21033 || useItemID == chargedItemID) ? useSlot : onSlot;
            var chargeWithItemID = 21028;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
                player.setAnimation(7406);
            }
        } else if (this.used(useItemID, onItemID, 13233, 11920) || this.used(useItemID, onItemID, 13233, 13244)
                || this.used(useItemID, onItemID, 13233, 13243)) {
            // Smouldering stone and Dragon pickaxe/Infernal pickaxe
            if (player.getController().getLevelForXP(Skills.SMITHING) < 85) {
                player.getGameEncoder().sendMessage("You need a Smithing level of 85 to do this.");
                return;
            }
            var chargedItemID = 13243;
            var chargingSlot = (useItemID == 11920 || useItemID == 13244 || useItemID == chargedItemID)
                    ? useSlot : onSlot;
            var chargeWithItemID = 13233;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
        ...
        } else if (this.used(useItemID, onItemID, 11920, 13243) || this.used(useItemID, onItemID, 11920, 13244)) {
            ...
            var chargedItemID = 13243;
            var chargingSlot = (useItemID == 13244 || useItemID == chargedItemID) ? useSlot : onSlot;
            var chargeWithItemID = 11920;
            if (player.getCharges().chargeFromInventory(chargedItemID, chargingSlot, 1, new Item(chargeWithItemID, 1),
                    ItemDef.getDegradeTime(chargedItemID)) > 0) {
                player.setAnimation(4514);
            }
        } else if (this.used(useItemID, onItemID, 4170, 21257) || this.used(useItemID, onItemID, 21255, 21257)) {
            // Slayer's staff and Slayer's enchantment
            var chargedItemID = 21255;
            var chargingSlot = (useItemID == 4170 ||  useItemID == 21255) ? useSlot : onSlot;
            var chargeWithItemID = 21257;
        ...
        } else if (this.used(useItemID, onItemID, 21817, 21820) || this.used(useItemID, onItemID, 21816, 21820)) {
            // Bracelet of ethereum and Revenant ether
            var chargedItemID = 21816;
            var chargingSlot = (useItemID == 21817 ||  useItemID == 21816) ? useSlot : onSlot;
            var chargeWithItemID = 21820;
        ...
        } else if (this.used(useItemID, onItemID, 22542, 21820) || this.used(useItemID, onItemID, 22545, 21820)) {
            // Viggora's chainmace and Revenant ether
            var chargedItemID = 22545;
            var chargingSlot = (useItemID == 22542 ||  useItemID == 22545) ? useSlot : onSlot;
            var chargeWithItemID = 21820;
            player.getCharges().chargeFromInventory(chargedItemID, chargingSlot,
                    player.getInventory().getCount(chargeWithItemID), new Item(chargeWithItemID, 1), 1);
        } else if (this.used(useItemID, onItemID, 22547, 21820) || this.used(useItemID, onItemID, 22550, 21820)) {
            // Craw's bow and Revenant ether
            var chargedItemID = 22550;
            var chargingSlot = (useItemID == 22547 ||  useItemID == 22550) ? useSlot : onSlot;
            var chargeWithItemID = 21820;
            player.getCharges().chargeFromInventory(chargedItemID, chargingSlot,
                    player.getInventory().getCount(chargeWithItemID), new Item(chargeWithItemID, 1), 1);
        } else if (this.used(useItemID, onItemID, 22552, 21820) || this.used(useItemID, onItemID, 22555, 21820)) {
            // Thammaron's sceptre and Revenant ether
            var chargedItemID = 22555;
            var chargingSlot = (useItemID == 22552 ||  useItemID == 22555) ? useSlot : onSlot;
            var chargeWithItemID = 21820;
            player.getCharges().chargeFromInventory(chargedItemID, chargingSlot,
                    player.getInventory().getCount(chargeWithItemID), new Item(chargeWithItemID, 1), 1);
        } else if (this.used(useItemID, onItemID, 1937, 1987)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 6570)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 21295)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 10499)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 2412)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 2414)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 2413)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 21791)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 21795)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 21793)) {
        ...
        } else if (this.used(useItemID, onItemID, 13280, 22109)) {
        ...
        } else if (this.used(useItemID, onItemID, 21270, 11866)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11867)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11868)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11869)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11870)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11871)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11872)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 21270, 11873)) {
            ...
            player.getInventory().setUpdate(true);
        } else if (this.used(useItemID, onItemID, 13280, 13124)) {
        ...
        } else if (this.used(useItemID, onItemID, 21730, 11836)) {
            // Black tourmaline core and Bandos boots
            player.getInventory().deleteItem(useItemID, 1, useSlot);
            player.getInventory().deleteItem(onItemID, 1, onSlot);
            player.getInventory().addItem(21733, 1, useSlot);
            player.getGameEncoder().sendMessage("The second you hold your tourmaline core by the Bandos boots, the core is absorbed rapidly. Perhaps there is some link between the metal in Bandos and tourmaline...");
        } else if (this.hasMatch(useItemID, onItemID, 11818, 11820, 11822)) {
            // Godsword shards
            ...
            if (player.getSkills().getLevel(Skills.SMITHING) < 80) {
                player.getGameEncoder().sendMessage("You need a Smithing level of 80 to do this.");
                return;
            }
        ...
        } else if (this.hasMatch(useItemID, onItemID, 8901)) {
            // Slayer helmet
        ...
        } else if (this.hasMatch(useItemID, onItemID, 8921)) {
            // Slayer helmet
            if (!player.getSlayer().isUnlocked(Slayer.MALEVOLENT_MASQUERADE)) {
                player.getGameEncoder().sendMessage("You need to unlock this feature through Slayer.");
                return;
            ...
            player.getInventory().deleteItem(4155, 1);
            player.getInventory().addItem(11864, 1, onSlot);
        } else if (this.hasMatch(useItemID, onItemID, 11774)) {
            // Slayer helmet (i)
            if (!player.getSlayer().isUnlocked(Slayer.MALEVOLENT_MASQUERADE)) {
                player.getGameEncoder().sendMessage("You need to unlock this feature through Slayer.");
                return;
        ...
        } else if (this.hasMatch(useItemID, onItemID, 12924, 12926)) {
            // Toxic blowpipe
        ...
        } else if (this.hasMatch(useItemID, onItemID, 12929, 12931)) {
            // Serpentine helm
        ...
        } else if (this.hasMatch(useItemID, onItemID, 13196, 13197)) {
            // Tanzanite helm
        ...
        } else if (this.hasMatch(useItemID, onItemID, 13198, 13199)) {
            // Magma helm
        ...
        } else if (this.hasMatch(useItemID, onItemID, 12902, 12904)) {
        ...
        } else if (this.hasMatch(useItemID, onItemID, 13274, 13275, 13276)) {
            // Bludgeon pieces
            if (!player.getInventory().hasItem(13274) || !player.getInventory().hasItem(13275)
                    || !player.getInventory().hasItem(13276)) {
                player.getGameEncoder().sendMessage("You don't have all the pieces to do this.");
                return;
            }
        ...
        } else if (this.hasMatch(useItemID, onItemID, 19679, 13275, 13276)) {
            // Dark totem pieces
        ...
        } else if (this.hasMatch(useItemID, onItemID, 12791)) {
            // Rune pouch
            if (useItemID == 12791) {
        ...
        } else if (this.hasMatch(useItemID, onItemID, 11941)) {
            // Looting bag
            var depositingID = (useItemID != 11941) ? useItemID : onItemID;
            var depositingSlot = (useItemID != 11941) ? useSlot : onSlot;
            ...
            } else if (player.getWidgetManager().getLootingBagStoreType()
                    == WidgetManager.LOOTING_BAG_STORE_ALL) {
            ...
            player.removeAttribute("looting_bag_item_slot");
        } else if (this.hasMatch(useItemID, onItemID, ItemID.HYDRAS_HEART, ItemID.HYDRAS_EYE, ItemID.HYDRAS_FANG)) {
...
DecodedPacketItemOnItem.prototype.used = function(useItemID, onItemID, itemID1, itemID2) {
    return useItemID == itemID1 && onItemID == itemID2 || useItemID == itemID2 && onItemID == itemID1;
}

DecodedPacketItemOnItem.prototype.hasMatch = function(useItemID, onItemID) {
    for (var i = 2; i < arguments.length; i++) {
        if (useItemID == arguments[i] || onItemID == arguments[i]) {
            return true;
        }
    }
    return false;
