DecodedPacketWidget.prototype.widget_149 = function(player, index, childID, slot, itemID) {
    ...
    if (index == 4 && (item.getDef().getOption(index) == null || item.getDef().isOption(index, "drop")
        ...
        RequestManager.addPlayerLog(player, "mapitem", player.getLogName() + " dropped " + item.getLogName()
                + " located at " + player + ".");
        return;
    }
    var height = 0;
    ...
    case ItemID.PERMANENT_TZHAAR_WAVE_BOOST_32300:
        player.getInventory().deleteItem(itemID, 1, slot);
        player.getTzHaar().setPermanentWaveBoost(true);
    ...
    case ItemID.RANDOM_BARROWS_PIECE:
        items = [
            ...
            ItemID.DHAROKS_HELM, ItemID.DHAROKS_GREATAXE, ItemID.DHAROKS_PLATEBODY, ItemID.DHAROKS_PLATELEGS,
            ItemID.GUTHANS_HELM, ItemID.GUTHANS_WARSPEAR, ItemID.GUTHANS_PLATEBODY, ItemID.GUTHANS_CHAINSKIRT,
            ItemID.KARILS_COIF, ItemID.KARILS_CROSSBOW, ItemID.KARILS_LEATHERTOP, ItemID.KARILS_LEATHERSKIRT,
            ItemID.TORAGS_HELM, ItemID.TORAGS_HAMMERS, ItemID.TORAGS_PLATEBODY, ItemID.TORAGS_PLATELEGS,
            ItemID.VERACS_HELM, ItemID.VERACS_FLAIL, ItemID.VERACS_BRASSARD, ItemID.VERACS_PLATESKIRT
        ];
        player.getInventory().deleteItem(itemID, 1, slot);
        randomItem = new Item(items[Utils.randomE(items.length)], 1);
        player.getInventory().addOrDropItem(randomItem);
        RequestManager.addPlayerLog(player, "lootbox", player.getLogName() + " received " + randomItem.getLogName()
                + " from a random barrows piece box.");
        break;
    case ItemID.RANDOM_PVP_WEAPON:
    case ItemID.RANDOM_PVP_WEAPON_32290:
        ...
        player.getInventory().deleteItem(itemID, 1, slot);
        randomItem = new Item(items[Utils.randomE(items.length)], 1);
        if (randomItem.getID() == ItemID.MORRIGANS_JAVELIN_32260
                || randomItem.getID() == ItemID.MORRIGANS_THROWING_AXE_32261) {
            randomItem.setAmount(50);
        }
        player.getInventory().addOrDropItem(randomItem);
        RequestManager.addPlayerLog(player, "lootbox", player.getLogName() + " received " + randomItem.getLogName()
                + " from a random pvp weapon box.");
        break;
    case ItemID.DARK_RELIC:
        ...
        for each (var raidsSkill in raidsSkills) {
    ...
    case 13226: // Herb sack
        if (index == 0) {
            for (var i = player.getInventory().size(); i >= 0; i--) {
            ...
            for (var i = player.getWidgetManager().getHerbSack().size(); i >= 0; i--) {
    ...
    case 13639: // Seed box
        if (index == 0) {
            ...
            for (var i = player.getWidgetManager().getSeedBox().size(); i >= 0; i--) {
    ...
    case 20703: // Supply crate
        ...
        player.getInventory().deleteItem(itemID, 1, slot);
        var supplyCount = 2 + Utils.randomI(2);
        for (var i = 0; i < supplyCount; i++) {
            if (Utils.inRange(player.getCombat().getDropRate(6739, 0.01))) {
                ...
                player.getInventory().addItem(RandomItem.getItem(items), slot);
            }
        }
        break;
    ...
    case ItemID.MYSTERY_BOX:
        ...
        var chancedItems = new ArrayList();
        for (var i = 0; i < 5; i++) {
            chancedItems.add(null);
        }
        var event = new Event(0) {
            execute: function() {
                ...
                chancedItems.remove(0);
                if (event.getExecutions() < 5) {
                    randomItem = MysteryBox.getBoxItem();
                    chancedItems.add(randomItem);
                }
                player.getGameEncoder().sendItems(WidgetID.MYSTERY, 48, 1, chancedItems);
                player.getSession().write();
                if (event.getExecutions() == 8) {
                    event.stop();
                    player.getInventory().addOrDropItem(new Item(randomItem.getID(), randomItem.getAmount()));
                    RequestManager.addPlayerLog(player, "mysterybox", player.getLogName() + " received "
                            + randomItem.getLogName() + " from a mystery box.");
                }
            }
        }
    ...
    case ItemID.SUPER_MYSTERY_BOX_32286:
        ...
        var event = new Event(0) {
            execute: function() {
                ...
                chancedItems.remove(0);
                if (event.getExecutions() < 5) {
                    randomItem = MysteryBox.getSuperBoxItem();
                    chancedItems.add(randomItem);
                }
                player.getGameEncoder().sendItems(WidgetID.MYSTERY, 48, 1, chancedItems);
                player.getSession().write();
                if (event.getExecutions() == 8) {
                    ...
                    player.getInventory().addOrDropItem(new Item(randomItem.getID(), randomItem.getAmount()));
                    RequestManager.addPlayerLog(player, "mysterybox", player.getLogName() + " received "
                            + randomItem.getLogName() + " from a super mystery box.");
                }
            }
        }
        player.getWorld().addEvent(event);
    ...
    case 19564: // Royal seed pod
        ...
        player.getMovement().animatedTeleport(new Tile(2465, 3495), 4544, 4546, new Graphic(767),
                new Graphic(769), 2);
    ...
    case 5515: // Giant pouch
        if (index == 0) {
            ...
            var addingAmount = player.getWidgetManager().getRCPouch(itemID).canAddAmount(7936, pureEssenceCount);
            ...
            var pureEssenceCount = player.getWidgetManager().getRCPouch(itemID).getCount(7936);
    ...
    case 21730: // Black tourmaline core
        player.getGameEncoder().sendMessage("Fallen from the centre of a Grotesque Guardian. This could be attached to a pair of Bandos boots...");
        break;
    ...
    case 748: // Legends Quest Holy force
        if (player.getCombat().getLegendsQuest() == 2 && player.getX() >= 2387 && player.getX() <= 2397
            ...
            var nezikchened = new Npc(player.getController(), 3962, new Tile(2396, 4678, player.getHeight()));
    ...
    case 11738: // Herb box
        ...
        var herbItems = [
            ...
            new RandomItem(201, 1) /* Marrentill */,
            new RandomItem(203, 1) /* Tarromin */,
            new RandomItem(205, 1) /* Harralander */,
        ...
        for (var i = 0; i < 30; i++) {
            var herbItem = RandomItem.getItem(herbItems);
    ...
    case ItemID.BAR_BOX_32302:
        player.getInventory().deleteItem(itemID, 1, slot);
        var barItems = [
            new RandomItem(2364, 1) /* Runite bar */,
            new RandomItem(2362, 1) /* Adamantite bar */,
            new RandomItem(2360, 1) /* Mithril bar */,
            new RandomItem(2354, 1) /* Steel bar */,
            new RandomItem(2352, 1) /* Iron bar */,
            new RandomItem(2350, 1) /* Bronze bar */
        ];
        for (var i = 0; i < 15; i++) {
            var barItem = RandomItem.getItem(barItems);
    ...
    case ItemID.BAG_FULL_OF_GEMS:
        ...
        var gemItems = [
            ...
            new RandomItem(ItemID.UNCUT_ONYX, 1).setWeight(8),
            new RandomItem(ItemID.UNCUT_ZENYTE, 1).setWeight(1)
        ];
        for (var i = 0; i < 20; i++) {
            var gemItem = RandomItem.getItem(gemItems);
    ...
    case ItemID.CLUE_NEST_EASY:
        ...
        var ttEasy = [
            ...
            ItemID.STEEL_PLATEBODY_T, ItemID.STEEL_PLATELEGS_T, ItemID.STEEL_PLATESKIRT_T, ItemID.STEEL_KITESHIELD_T,
            ItemID.STEEL_FULL_HELM_G, ItemID.STEEL_PLATEBODY_G, ItemID.STEEL_PLATELEGS_G, ItemID.STEEL_PLATESKIRT_G,
            ItemID.STEEL_KITESHIELD_G, ItemID.BLACK_FULL_HELM_T, ItemID.BLACK_PLATEBODY_T, ItemID.BLACK_PLATELEGS_T,
            ItemID.BLACK_PLATESKIRT_T, ItemID.BLACK_KITESHIELD_T, ItemID.BLACK_FULL_HELM_G, ItemID.BLACK_PLATEBODY_G,
            ItemID.BLACK_PLATELEGS_G, ItemID.BLACK_PLATESKIRT_G, ItemID.BLACK_KITESHIELD_G, ItemID.BLACK_BERET,
            ItemID.BLUE_BERET, ItemID.WHITE_BERET, ItemID.RED_BERET, ItemID.HIGHWAYMAN_MASK, ItemID.BEANIE,
            ItemID.BLUE_WIZARD_HAT_T, ItemID.BLUE_WIZARD_ROBE_T, ItemID.BLUE_SKIRT_T, ItemID.BLUE_WIZARD_HAT_G,
            ItemID.BLUE_WIZARD_ROBE_G, ItemID.BLUE_SKIRT_G, ItemID.BLACK_WIZARD_HAT_T, ItemID.BLACK_WIZARD_ROBE_T,
            ItemID.BLACK_SKIRT_T, ItemID.BLACK_WIZARD_HAT_G, ItemID.BLACK_WIZARD_ROBE_G, ItemID.BLACK_SKIRT_G,
            ItemID.STUDDED_BODY_T, ItemID.STUDDED_CHAPS_T, ItemID.STUDDED_BODY_G, ItemID.STUDDED_CHAPS_G,
            ItemID.BLACK_HELM_H1, ItemID.BLACK_HELM_H2, ItemID.BLACK_HELM_H3, ItemID.BLACK_HELM_H4,
            ItemID.BLACK_HELM_H5, ItemID.BLACK_PLATEBODY_H1, ItemID.BLACK_PLATEBODY_H2, ItemID.BLACK_PLATEBODY_H3,
            ...
            ItemID.GREEN_ELEGANT_SKIRT, ItemID.RED_ELEGANT_SHIRT, ItemID.RED_ELEGANT_LEGS, ItemID.RED_ELEGANT_BLOUSE,
            ItemID.RED_ELEGANT_SKIRT, ItemID.BOBS_RED_SHIRT, ItemID.BOBS_BLUE_SHIRT, ItemID.BOBS_GREEN_SHIRT,
            ItemID.BOBS_BLACK_SHIRT, ItemID.BOBS_PURPLE_SHIRT, ItemID.STAFF_OF_BOB_THE_CAT, ItemID.A_POWDERED_WIG,
            ItemID.FLARED_TROUSERS, ItemID.PANTALOONS, ItemID.SLEEPING_CAP, ItemID.AMULET_OF_MAGIC_T,
            ItemID.AMULET_OF_POWER_T, ItemID.RAIN_BOW, ItemID.HAM_JOINT, ItemID.BLACK_CANE, ItemID.BLACK_PICKAXE,
            ItemID.GUTHIX_ROBE_TOP, ItemID.GUTHIX_ROBE_LEGS, ItemID.SARADOMIN_ROBE_TOP, ItemID.SARADOMIN_ROBE_LEGS,
            ItemID.ZAMORAK_ROBE_TOP, ItemID.ZAMORAK_ROBE_LEGS, ItemID.ANCIENT_ROBE_TOP, ItemID.BANDOS_ROBE_LEGS,
            ...
            ItemID.WOODEN_SHIELD_G, ItemID.GOLDEN_CHEFS_HAT, ItemID.GOLDEN_APRON, ItemID.MONKS_ROBE_TOP_G,
            ItemID.MONKS_ROBE_G, ItemID.LARGE_SPADE
        ];
        player.getInventory().deleteItem(itemID, 1, slot);
        var easyItemID = ttEasy[Utils.randomE(ttEasy.length)];
    ...
    case ItemID.CLUE_NEST_HARD:
        ...
        var ttHard = [
            ...
            ItemID.ZAMORAK_PLATELEGS, ItemID.ZAMORAK_PLATESKIRT, ItemID.ZAMORAK_KITESHIELD, ItemID.ANCIENT_FULL_HELM,
            ItemID.ANCIENT_PLATEBODY, ItemID.ANCIENT_PLATELEGS, ItemID.ANCIENT_PLATESKIRT, ItemID.ANCIENT_KITESHIELD,
            ItemID.BANDOS_FULL_HELM, ItemID.BANDOS_PLATEBODY, ItemID.BANDOS_PLATELEGS, ItemID.BANDOS_PLATESKIRT,
            ItemID.BANDOS_KITESHIELD, ItemID.ARMADYL_FULL_HELM, ItemID.ARMADYL_PLATEBODY, ItemID.ARMADYL_PLATELEGS,
            ItemID.ARMADYL_PLATESKIRT, ItemID.ARMADYL_KITESHIELD, ItemID.RUNE_HELM_H1, ItemID.RUNE_HELM_H2,
            ItemID.RUNE_HELM_H3, ItemID.RUNE_HELM_H4, ItemID.RUNE_HELM_H5, ItemID.RUNE_PLATEBODY_H1,
            ItemID.RUNE_PLATEBODY_H2, ItemID.RUNE_PLATEBODY_H3, ItemID.RUNE_PLATEBODY_H4, ItemID.RUNE_PLATEBODY_H5,
            ItemID.BLUE_DHIDE_BODY_T, ItemID.BLUE_DHIDE_CHAPS_T, ItemID.BLUE_DHIDE_BODY_G, ItemID.BLUE_DHIDE_CHAPS_G,
            ItemID.RED_DHIDE_BODY_T, ItemID.RED_DHIDE_CHAPS_T, ItemID.RED_DHIDE_BODY_G, ItemID.RED_DHIDE_CHAPS_G,
            ItemID.ENCHANTED_HAT, ItemID.ENCHANTED_TOP, ItemID.ENCHANTED_ROBE, ItemID.ROBIN_HOOD_HAT,
            ItemID.TAN_CAVALIER, ItemID.DARK_CAVALIER, ItemID.BLACK_CAVALIER, ItemID.WHITE_CAVALIER,
            ItemID.RED_CAVALIER, ItemID.NAVY_CAVALIER, ItemID.PIRATES_HAT, ItemID.AMULET_OF_GLORY_T4,
            ItemID.GUTHIX_COIF, ItemID.GUTHIX_DHIDE, ItemID.GUTHIX_CHAPS, ItemID.GUTHIX_BRACERS,
            ItemID.GUTHIX_DHIDE_BOOTS, ItemID.GUTHIX_DHIDE_SHIELD, ItemID.SARADOMIN_COIF,
            ItemID.SARADOMIN_DHIDE, ItemID.SARADOMIN_CHAPS, ItemID.SARADOMIN_BRACERS,
            ItemID.SARADOMIN_DHIDE_BOOTS, ItemID.SARADOMIN_DHIDE_SHIELD, ItemID.ZAMORAK_COIF,
            ItemID.ZAMORAK_DHIDE, ItemID.ZAMORAK_CHAPS, ItemID.ZAMORAK_BRACERS, ItemID.ZAMORAK_DHIDE_BOOTS,
            ItemID.ZAMORAK_DHIDE_SHIELD, ItemID.ARMADYL_COIF, ItemID.ARMADYL_DHIDE, ItemID.ARMADYL_CHAPS,
            ItemID.ARMADYL_BRACERS, ItemID.ARMADYL_DHIDE_BOOTS, ItemID.ARMADYL_DHIDE_SHIELD, ItemID.ANCIENT_COIF,
            ItemID.ANCIENT_DHIDE, ItemID.ANCIENT_CHAPS, ItemID.ANCIENT_BRACERS, ItemID.ANCIENT_DHIDE_BOOTS,
            ItemID.ANCIENT_DHIDE_SHIELD, ItemID.BANDOS_COIF, ItemID.BANDOS_DHIDE, ItemID.BANDOS_CHAPS,
            ItemID.BANDOS_BRACERS, ItemID.BANDOS_DHIDE_BOOTS, ItemID.BANDOS_DHIDE_SHIELD, ItemID.GUTHIX_STOLE,
            ItemID.SARADOMIN_STOLE, ItemID.ZAMORAK_STOLE, ItemID.GUTHIX_CROZIER, ItemID.SARADOMIN_CROZIER,
            ItemID.ZAMORAK_CROZIER, ItemID.GREEN_DRAGON_MASK, ItemID.BLUE_DRAGON_MASK, ItemID.RED_DRAGON_MASK,
            ItemID.BLACK_DRAGON_MASK, ItemID.PITH_HELMET, ItemID.EXPLORER_BACKPACK, ItemID.RUNE_CANE,
            ItemID.ZOMBIE_HEAD_19912, ItemID.CYCLOPS_HEAD, ItemID.NUNCHAKU, ItemID.DUAL_SAI, ItemID.THIEVING_BAG,
            ItemID.DRAGON_BOOTS_ORNAMENT_KIT, ItemID.RUNE_DEFENDER_ORNAMENT_KIT, ItemID.TZHAAR_KET_OM_ORNAMENT_KIT,
    ...
    case ItemID.CLUE_NEST_ELITE:
        ...
        player.getInventory().deleteItem(itemID, 1, slot);
        if (Utils.inRange(0.1)) {
            var thirdAgeID = thirdageElite[Utils.randomE(thirdageElite.length)];
            player.getInventory().addItem(thirdAgeID, 1, slot);
            player.getWorld().sendItemDropNews(player, thirdAgeID);
        } else if (Utils.inRange(0.5)) {
            var gildedID = gildedElite[Utils.randomE(gildedElite.length)];
            player.getInventory().addItem(gildedID, 1, slot);
            player.getWorld().sendItemDropNews(player, gildedID);
        } else {
            var eliteItemID = ttElite[Utils.randomE(ttElite.length)];
    ...
    case ItemID.CLUE_SCROLL_MASTER:
        ...
        player.getInventory().deleteItem(itemID, 1, slot);
        if (Utils.inRange(0.1)) {
            var thirdAgeID = thirdageMaster[Utils.randomE(thirdageMaster.length)];
            player.getInventory().addItem(thirdAgeID, 1, slot);
            player.getWorld().sendItemDropNews(player, thirdAgeID);
        } else if (Utils.inRange(0.5)) {
            var gildedID = gildedMaster[Utils.randomE(gildedMaster.length)];
            player.getInventory().addItem(gildedID, 1, slot);
            player.getWorld().sendItemDropNews(player, gildedID);
        } else {
            var masterItemID = ttMaster[Utils.randomE(ttMaster.length)];
    ...
    case 6543: // Antique lamp
        ...
        var herbloreXP = (12.5 * (player.getController().getLevelForXP(Skills.HERBLORE)
                * player.getController().getLevelForXP(Skills.HERBLORE)
                - player.getController().getLevelForXP(Skills.HERBLORE) * 2 + 100) * 2)|0;
        var miningXP = (12.5 * (player.getController().getLevelForXP(Skills.MINING)
                * player.getController().getLevelForXP(Skills.MINING)
                - player.getController().getLevelForXP(Skills.MINING) * 2 + 100) * 2)|0;
        var smithingXP = (12.5 * (player.getController().getLevelForXP(Skills.SMITHING)
                * player.getController().getLevelForXP(Skills.SMITHING)
                - player.getController().getLevelForXP(Skills.SMITHING) * 2 + 100) * 2)|0;
    ...
    case ItemID.OLD_SCHOOL_BOND_UNTRADEABLE:
        player.getInventory().deleteItem(itemID, 1, slot);
        player.getInventory().addItem(_14_DAYS_GOLD_MEMBERSHIP_32303, 1, slot);
        break;
    ...
    case 5521: // Binding necklace
        player.getGameEncoder().sendMessage("Your Binding Necklace has " + player.getSkills().getBindingNecklace()
                + " charges remaining.");
        break;
    ...
    case 11238: // Baby impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(babyImpling));
        break;
    case 11240: // Young impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(youngImpling));
        break;
    case 11242: // Gourmet impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(gourmetImpling));
        break;
    case 11244: // Earth impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(earthImpling));
        break;
    case 11246: // Essence impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(essenceImpling));
        break;
    case 11248: // Eclectic impling jar
        ...
        var eclecticImpling = [
            ...
            new RandomItem(1213, 1) /* Rune dagger */,
            new RandomItem(1391, 1) /* Battlestaff */,
            new RandomItem(5970, 1) /* Curry leaf */,
            new RandomItem(231, 1) /* Snape grass */,
    ...
    case 11250: // Nature impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(natureImpling));
        break;
    case 11252: // Magpie impling jar
        ...
        var magpieImpling = [
            ...
            new RandomItem(4095, 1) /* Mystic gloves */,
            ...
            new RandomItem(993, 1) /* Sinister key */,
            new RandomItem(5300, 1) /* Snapdragon seed */
        ];
        player.getInventory().addOrDropItem(RandomItem.getItem(magpieImpling));
        break;
    case 11254: // Ninja impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(ninjaImpling));
        break;
    case 11256: // Dragon impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(dragonImpling));
        break;
    case 19732: // Lucky impling jar
        ...
        player.getInventory().addOrDropItem(RandomItem.getItem(luckyImpling));
        break;
    case 19675: // Arclight
    ...
    case ItemID.TORMENTED_BRACELET_OR:
        ...
        player.getInventory().deleteItem(itemID, 1, slot);
        for each (var itemID in item.getDef().getExchangeIDs()) {
            player.getInventory().addItem(itemID, 1, slot);
        }
        break;
    case Runecrafting.Altar.AIR.talismanID:
    case Runecrafting.Altar.EARTH.talismanID:
    case Runecrafting.Altar.FIRE.talismanID:
    case Runecrafting.Altar.WATER.talismanID:
    case Runecrafting.Altar.BODY.talismanID:
    case Runecrafting.Altar.MIND.talismanID:
    case Runecrafting.Altar.CHAOS.talismanID:
    case Runecrafting.Altar.COSMIC.talismanID:
    case Runecrafting.Altar.DEATH.talismanID:
    case Runecrafting.Altar.NATURE.talismanID:
    case Runecrafting.Altar.WRATH.talismanID:
